...
class RemoteDataSourceTest {
    ...
    private val openExchangeRatesApi: OpenExchangeRatesApi = mock()

    @Before
    fun setUp() {
        remoteDataSource = RemoteDataSource(openExchangeRatesApi)
    }
    ...
    fun getLatestRatesSuccessTest() {
        ...
        val expected: Response<JsonObject> = Response.success(jsonObject)
        openExchangeRatesApi.stub {
            onBlocking { getAllCurrencyRateBasedOnUSD() }.doReturn(expected)
        }
        runBlocking {
            val response = remoteDataSource.getLatestRates()
            assertTrue(response is Resource.Success)
            assertEquals(jsonObject, response.data)
        }
    }

    @Test
    fun getLatestRatesErrorTest() {
        val expected: Response<JsonObject> = Response.error(
            403,
            "{\"key\":[\"somestuff\"]}"
